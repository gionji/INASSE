...
def main():
    ...
    parser.add_argument('--modbus-cc-units',
                        action='store',
                        default=DEFAULT_CHARGE_CONTROLLER_UNITS,
                        dest='modbusCcUnits',
                        nargs='*',
                        type=str,
                        help='Set the Charge Controller UNITS numners')

    parser.add_argument('--modbus-rb-units',
                        action='store',
                        default=DEFAULT_RELAYBOX_UNITS,
                        dest='modbusRbUnits',
                        nargs='*',
                        type=str,
                        help='Set the Relay Box UNITS numners')
...
#                   help='Set the Charge Controller RS485 Serial port')
    ...
    CHARGE_CONTROLLER_UNITS = results.modbusCcUnits
    RELAYBOX_UNITS         = results.modbusRbUnits
    ...
    if IS_MODBUS_IN_DEBUG_MODE == True:
        ...
        print ("Charge Controller Units: " + str(CHARGE_CONTROLLER_UNITS))
        print ("RelayBox Units: " + str(RELAYBOX_UNITS))
    ...
    try:
        ...
        chargeController = FdsCC.FdsChargeController(FdsCC.MODBUS_ETH,
                                                     isDebug = IS_MODBUS_IN_DEBUG_MODE,
                                                     ipAddress = MODBUS_IP )
